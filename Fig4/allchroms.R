...
#sigat0025rect_phys <- geom_rect(data=sigregions0025, aes(xmin=phystart/1000000, xmax=phystop/1000000, ymin=0, ymax=0.5), fill="red") 
Hplot_phys <- geom_point(size = 1, data=Hdata, aes(x=physpos/1000000, y=HetSS, colour=sqrt(rate^(1/4)))) 
myphysplot=ggplot(data=Hdata) + facet_grid(Cycle~.) + sigat001rect_phys + Hplot_phys  + xlab("Position(Mb)")+ ylab("Heterozygosity") + scale_colour_gradient2(low="red", mid="orange", high="yellow", midpoint=median(Hdata$rate^(1/4))) + theme_bw() + ggtitle(paste("BSSS, Chromosome ", Chrom, sep=''))
...
#simdata2rect_gen <- geom_rect(data=sigregions0025, aes(xmin=genstart, xmax=genstop, ymin=0, ymax=0.5), fill="red") 
...
ChrBSSS = list(phys=myphysplot, gen=mygenplot)
...
myphysplot=ggplot(data=Hdata) + facet_grid(Cycle~.) + sigat001rect_phys + Hplot_phys + xlab("Position(Mb)")+ ylab("Heterozygosity") + scale_colour_gradient2(low="red", mid="orange", high="yellow", midpoint=median(Hdata$rate^(1/4))) + theme_bw() + ggtitle(paste("BSCB1, Chromosome ", Chrom, sep=''))
...
ChrBSCB1 = list(phys=myphysplot, gen=mygenplot)
