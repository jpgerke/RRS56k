...
\response{We feel that while an experiment-wide p-value would be important if we were performing a formal genome scan, our purpose here was simply to show that while our overall power to distinguish selection from drift may be low, there are nonetheless individual genomic regions that show patterns of fixation suggestive of selection. We report the \% of simulations for each region that showed lower heterozygosity in simulations, so a simple bonferroni or other calculation could be calculated by a reader concerned we are not sufficiently conservative. \jri{Do we want to do an FDR for the p-values in Figure 3?}}
...
\response{This is a good point and we have added a few words in the intro in the hopes of making this clearer.}
...
\response{We had avoided citing \citet{jiao2012genome} because we were  unsure of the veracity of the results (see \citet{lorenz2013phylogenetic}) and concerned by the fact that a wholesale recoding of their genotyping data appeared to have little impact on their conclusions. We have now added a citation, however, as it is certainly true that they performed similar analyses.}
