...
melted_SS_all = melt(simresults)
colnames(melted_SS_all) = c("Cycle", "H")
melted_SS_all$Pruning = "all"

colnames(melted_SS_all) = c("Cycle",  "H", "Pruning")
medians = plyr::ddply(.data=melted_SS_all, .variables=c("Cycle", "Pruning"), .fun=function(x){median(x[,2])})
...
bounds = plyr::ddply(.data=melted_SS_all, .variables=c("Cycle", "Pruning"), .fun=getbounds)
colnames(bounds) = c("Cycle", "Pruning", "one", "ninenine")
realdata = realdata[realdata$Pruning=="all",]
#png("../FigS1A.png", width=400,height=300)
myplot = ggplot()
myplot = myplot + geom_point(data=melted_SS_all, aes(x=Cycle, y=H), alpha=0.1) + geom_line(data=bounds, aes(x=Cycle, y=one, group=Pruning), color="black") + geom_line(data=bounds, aes(x=Cycle, y=ninenine, group=Pruning), color="black")
...
plot_BSSS = myplot + theme(axis.title.x=element_text(size=0)) + scale_y_continuous(limits=c(0.1,0.3))
save(plot_BSSS, file="plot_BSSS.rds")
#dev.off()
