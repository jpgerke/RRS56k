...
The vast majority of regions showing such evidence of selection differ between the two populations, providing evidence against the overdominance model as a means of explaining selection for hybrid vigor. }
...
\rev{In this study, we examine a different closed experiment whose phenotypic target and selection method --- reciprocal recurrent selection --- closely mirrors methods used in the generation of modern maize hybrids \citep{comstock1949breeding, duvick2004long}. 
Proposed by \citet{comstock1949breeding}, reciprocal recurrent selection is a procedure to simultaneously improve two populations for their ability to produce high yielding hybrids when individuals from the two populations are crossed. 
In this method, the two populations are kept separate and lines from each population are selected based on the phenotype of their cross-population hybrids. 
Selected lines within each population are then intermated to generate lines for the next cycle. 
...
This set of resources on a closed population selected for hybrid yield make the Iowa RRS an ideal test case for the study of the genomic impact of intensive hybrid breeding in maize.} 
...
This drift was driven by inbreeding through self-pollination and the small effective population sizes used to select a limited number of high-performing, potentially related individuals at each cycle.  \jri{drosophila here? lamkey again replication isn't enough}	
...
\jri{likely these are not strongly deleterious, but lots of weakly deleterious}
