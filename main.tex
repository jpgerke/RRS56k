...
The lack of distinct selection signals \rev{in broad germplasm collections may be due to a population specific selection within germplasm subgroups, which would be further obscured by migration due to the exchange of material among breeders.} \textcolor{red}{cite Mikel here}  \rev{In this case selection will be easier to detect in populations maintained in an individual program.} This \rev{possibliity} is supported by the measured success in identifying targets of selection in individual experimental populations \rev{under directional selection for specific phenotypes such as seed size and ear number.} \citep{hirsch2014insights, beissinger2014genome}. 

\rev{In this study, we examine a different closed experiment whose phenotypic target and selection method -Reciprocal Recurrent Selection- closely mirrors methods used in the generation of modern maize hybrids \citep{comstock1949breeding, duvick2004long}. Proposed by Comstock et al. (1949), reciprocal recurrent selection is a procedure to simultaneously improve two populations for their ability to produce high yielding hybrids when individuals from the two populations are crossed. In this method, the two populations are kept separate and lines from each population are advanced to the next cycle of selection based on the performance of the line or its close relatives in a testcross with random individuals from the opposing population. Selection is therefore carried out on lines within each population, but on a cross-population hybrid phenotype. After selection the selected lines within each population are intermated to generate lines for the next cycle. In this way the two populations simultaneously evolve improved combining ability with one another. The USDA-ARS at Ames, Iowa has conducted a Reciprocal Recurrent Selection Program (hereafter referred to as the Iowa RRS) with the Iowa Stiff Stalk Synthetic (BSSS) and Iowa Corn Borer Synthetic No. 1 (BSCB1) populations for 18 cycles of selection\citep{penny1971twenty}} \textcolor{red}{cite Jode 2011 here}. \rev{This program represents one of the best-documented public experiments on selection for combining ability and hybrid performance. Reminiscent of modern maize germplasm as a whole, BSSS and BSCB1 have evolved from undifferentiated founders as they have been selected for cross-population hybrid yield. BSSS and BSCB1 have both been maintained as closed populations with no intentional migration (although evidence suggests low level of unintentional gene flow early in the BSSS population). Extensive records on population sizes, breeding methods, selection differentials, and seed are available for all cycles of selection. This set of resources on a closed population selected for hybrid yield make the Iowa RRS an ideal test case for the study of the genomic impact of intensive hybrid breeding in maize.} The Iowa RRS experiment \rev{provides additional historical relevance} because lines derived from the BSSS population have had a major impact upon the development of commercial hybrids \citep{duvick2004long, darrah1986}, the formation of modern heterotic groups \citep{troyer1999background, senior1998utility}, and the choice of a maize reference genome \citep{schnable2009the-b73-maize}.
\jri{something about heterosis} \jpg{what are you thinking for heterosis here?}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% INTRO
...
There is no separation between the founders of the two populations, \rev{and projection of later cycles onto a PCA of the founders shows no distinction between BSSS and BSCB1.
...
Nonetheless, we do observe \rev{differences from values generated under our neutral simulations} (Figure \ref{fig:compare_to_sims})\jri{is my interpretation of the black lines in figure correct?}. 
