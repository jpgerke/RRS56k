...
These patterns differ markedly, however, from similar experimental evolution in systems such as \emph{Drosophila} \citep{turner2011population, burke2010genome} and \emph{E. coli} \citep{tenaillon2012molecular} in which the effects of selection on diversity are readily discernible. 
A key difference between these studies and those in maize are the effective population sizes, which were kept much lower to produce a short-term phenotypic response to selection within available field testing resources.} \jpg{My intent in rewording this is to emphasize that Ne is the important difference between the scenarios.}
...
In the case of true overdominance, selection should thus lead to decreased heterozygosity at the same locus in both populations as complementary haplotypes are fixed in each group \rev{\citep[e.g.][]{guo2014maize}}. \jpg{One possible criticism is that overdominance could still be happening when each allele is on multiple founder haplotypes. Could say something like "Although we cannot rule out soft sweeps of complementary overdominant alleles, the observed pattern more parsimoniously favors..."}
...
Although our results better fit the simpler dominance model, we caution that the exact outcome of particular models will depend strongly on the effects of selection, drift, and the frequencies of beneficial alleles. 
...
\jpg{I'm not sure where this paragraph belongs.}Similar patterns of diversity and population structure can be seen broadly across North American maize germplasm \citep{van2012historical}, suggesting that genetic drift has most likely played a large role in the current genetic structure of modern maize.  
