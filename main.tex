...
The United States Department of Agriculture reciprocal recurrent selection experiment between the Iowa Stiff Stalk Synthetic (BSSS) and the Iowa Corn Borer Synthetic No. 1 (BSCB1) populations represents one of the longest \rev{running experiments to understand the genetic bases} for hybrid performance. 
...
Proposed by \citet{comstock1949breeding}, reciprocal recurrent selection simultaneously improves two populations for their ability to produce high yielding hybrids when individuals from the two populations are crossed. 
In this method, the two populations are isolated genetically and lines from each population are selected based on the phenotype of their cross-population hybrids. 
The superio lines within each population are then intermated to generate lines for the next cycle. 
In this way, the two populations simultaneously evolve improved combining ability with one another. 
The USDA-ARS at Ames, Iowa has conducted a reciprocal recurrent selection program (hereafter referred to as the Iowa RRS) with the Iowa Stiff Stalk Synthetic (BSSS) and Iowa Corn Borer Synthetic No. 1 (BSCB1) populations for 18 cycles of selection \citep{penny1971twenty}. 
...
Founder lines (Table \ref{tab:founders}) from each population were randomly mated to create the `cycle 0' base population.
...
The plants and inbred lines used in this experiment are listed in Table \ref{tab:founders}. 
...
Reciprocal recurrent selection serves as a model for the method of hybrid maize improvement \citep{duvick2004long} and similar patterns of diversity and population structure can be seen broadly across North American maize germplasm \citep{van2012historical}, suggesting , genetic drift has most likely played a large role in the current genetic structure of modern maize.  	
...
Near isogenic lines that substitute these centromeric haplotypes from key inbreds could be used to determine if the haplotypes at high frequency in cycle 16 of BSSS provide a phenotypic advantage when crossed with BSCB1. \jri{removed paragraph on drift at cycle 0. shortens discussion, felt out of place and we mention both possibilities already in the results}
...
%If this differentiation reflects drift during population maintenance before initiation of the RRS, it may have impacted the trajectory of the RRS program and its derived inbreds. 
...
Since these same trends are observed across wider North American germplasm \citep{tollenaar1989genetic}, identification and characterization of loci conferring phenotypic improvement in BSSS and BSCB1 could reveal some of the genetic mechanisms by which maize yield has steadily improved over the decades. 
...
\citet{van2012historical} assessed variation in and tested for selection across a wide range of North American maize germplasm using the same SNP array. 
In spite of the large genetic gains made in decades of maize breeding \citep{duvick2005contribution}, \citet{van2012historical} identified  few loci showing  selected loci were identified in that study, the overall effects on genomic diversity and haplotype patterns were relatively small compared with the effects of genetic drift. 
One explanation for the limited overall impact of selection could arise because different haplotypes and loci are selected in different breeding programs. 
